  ...
  value_force2_(NULL)
{
  ...
  if(in_restart_name_ != "") {
    ...
    if(last_update_step_==-1){
        log.printf("  Updating statistics only once per time step (new Sept 2023)\n");
    }
...
void FISST::calculate() {
  ...
  // check last_update_step_ to account for repeated calls to calculate during replica exchange simulations
  if( ! b_freeze_ && last_update_step_!=getStep() ) {
